#!/usr/bin/env python3
"""Launch a single sglang stage and surface startup issues.

Usage:

    python3 tools/debug_sglang_stage.py --stage prefill

The script reuses the settings in ``config/sglang_batch.yaml`` so you can
reproduce exactly what ``tools/run_sglang_batch.py`` would do, but for one
...
from tools.run_sglang_batch import (  # noqa: E402
...
def debug_stage(config_path: Path, stage: str, *, log_tail: int = 80, keep_alive: bool = False) -> int:
    ...
    sglang_cfg = ensure_mapping(cfg.get("sglang"), "sglang")
    stages = {label: entry for label, entry in _normalize_server_configs(sglang_cfg)}
    ...
    merged_env = _merge_two_envs(sglang_cfg.get("env"), stage_cfg.get("env"))
    command = build_sglang_command(stage_cfg)
    ...
    host = str(
        _resolve_server_value(
            ...
            sglang_cfg,
            "connect_host",
            _resolve_server_value(stage_cfg, sglang_cfg, "host", "127.0.0.1"),
        )
    )
    port = int(_resolve_server_value(stage_cfg, sglang_cfg, "port", 30000))
    timeout = int(_resolve_server_value(stage_cfg, sglang_cfg, "wait_timeout", cfg.get("wait_timeout", 180)))
    health_path = str(_resolve_server_value(stage_cfg, sglang_cfg, "health_path", "/v1/models"))
    ...
    print(f"[debug] Waiting for http://{host}:{port}{health_path} (timeout {timeout}s)\n")

    tmpdir = tempfile.mkdtemp(prefix=f"sglang-{stage}-debug-")
    log_path = Path(tmpdir) / "sglang.log"
    ...
    try:
        ...
        try:
            wait_for_server(host, port, timeout, path=health_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diagnose a single sglang stage defined in sglang_batch.yaml")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("config/sglang_batch.yaml"),
