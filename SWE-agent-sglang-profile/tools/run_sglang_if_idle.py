#!/usr/bin/env python3
"""Launch run_sglang_batch.py for selected runs when all GPUs are idle."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check GPU utilisation and trigger run_sglang_batch.py if GPUs are idle.",
    )
    parser.add_argument(
        "--config-file",
        type=Path,
        default=Path("config/sglang_batch.yaml"),
        help="Configuration file passed to tools/run_sglang_batch.py (default: %(default)s).",
    )
    parser.add_argument(
        "--runs",
        nargs="*",
...
def main() -> int:
    ...
    if exit_code != 0:
        print(f"run_sglang_batch.py exited with code {exit_code}", file=sys.stderr)
    return exit_code
