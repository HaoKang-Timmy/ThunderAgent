#!/usr/bin/env python3
"""Run local sglang batches without step synchronization."""
...
import run_sglang_batch as blocking
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Start sglang server and run sweagent batches without step synchronization.",
    )
    parser.add_argument(
        "--config-file",
        default="config/sglang_batch.yaml",
        help="Path to the YAML config describing sglang server and batch options.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    server_cfg = blocking.ensure_mapping(raw_config.get("sglang"), "sglang")
    ...
    server_cmd = blocking.build_sglang_command(server_cfg)
    ...
    try:
        ...
        blocking.wait_for_server(connect_host, port, timeout, path=health_path)
        print("sglang server is ready. Running sweagent batch (non-blocking)...")
