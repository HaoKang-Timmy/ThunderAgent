"""Hook to track LLM reasoning time per step (without querying backend metrics)."""
...
class PrefixCacheMetricsHook(AbstractAgentHook):
    """
    Captures wall-clock/perf timings around each model call for streaming-compatible
    backends. For every agent step, the hook tracks when a request starts, the
    first streamed token arrives, and the last chunk is received. Logs are saved
    to <instance_dir>/prefix_cache_metrics.jsonl with format:
    {"step": 0, "timestamp": 1234567890.123, "prefill_time": 0.42, "decode_time": 1.73}
    """
    ...
    def _write_record(self) -> None:
        ...
        record = {
            ...
            "sglang_prefill_tokens": getattr(model, "prefill_tokens", None),
            "sglang_cache_hit_tokens": getattr(model, "cache_hit_tokens", None),
            "sglang_new_tokens": getattr(model, "new_prompt_tokens", None),
            "sglang_decode_tokens": getattr(model, "decode_tokens", None),
        }
