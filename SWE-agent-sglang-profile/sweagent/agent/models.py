...
class GenericAPIModelConfig(PydanticBaseModel):
    ...
    """Override provider passed to litellm (e.g., `openai`, `sglang`)."""
...
class SGLangModelConfig(GenericAPIModelConfig):
    type: Literal["sglang"] = Field(description="Use a locally hosted sglang HTTP server.")
    api_base: str = Field(
        default="http://localhost:30000/v1",
        description="Base URL of the sglang OpenAI-compatible server.",
    )
    per_instance_cost_limit: float = 0.0
    total_cost_limit: float = 0.0
    choose_api_key_by_thread: bool = False
    custom_llm_provider: str | None = Field(default="openai", description="Provider name passed to litellm.")
    api_key: SecretStr | None = None
...
class AbstractModel(ABC):
    ...
    def register_first_token_callback(self, callback: Callable[[float, float], None]) -> None:
        callbacks = getattr(self, "_first_token_callbacks", None)
        if callbacks is None:
            callbacks = []
            setattr(self, "_first_token_callbacks", callbacks)
        callbacks.append(callback)

    def _notify_first_token(self, perf_time: float, wall_time: float) -> None:
        callbacks = getattr(self, "_first_token_callbacks", [])
        if not callbacks:
            return
        for callback in callbacks:
            try:
                callback(perf_time, wall_time)
            except Exception:
                continue
...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        ...
        if self.config.custom_llm_provider:
            extra_args["custom_llm_provider"] = self.config.custom_llm_provider
        ...
        use_stream = isinstance(self.config, SGLangModelConfig)
        if use_stream:
            outputs, output_tokens = self._single_query_stream(
                messages,
    ...
    def _single_query_stream(
        ...
        *,
        ...
        tool_calls_map: Dict[int, List[Dict[str, Any]]] = {}
        chunk_wall_times: List[float] = []
        first_token_emitted = False
        first_payload_wall: float | None = None
        last_chunk_wall_time: float | None = None
        setattr(self, "_last_chunk_wall_time", None)
        setattr(self, "_last_first_chunk_wall", None)
        self.prefill_tokens = None           # prompt_tokens
        self.cache_hit_tokens = None         # prompt_cache_hit_tokens or details.cached_tokens
        self.new_prompt_tokens = None        # prompt_cache_miss_tokens (= prefill - hit)
        self.decode_tokens = None            # completion_tokens
        self.total_tokens = None             # total_tokens
        try:
        ...
        for chunk in stream:
            ...
            # >>> NEW: 如果这是最后一个 chunk，会带 usage（前提：请求时设置了 stream_options={"include_usage": True}）
            # 同时兼容对象属性和 dict 两种形态
            if hasattr(chunk, "usage") and getattr(chunk, "usage") is not None:
                u = getattr(chunk, "usage")
                # 统一成 dict，便于 key 检索
                if hasattr(u, "model_dump"):
                    usage_raw = u.model_dump()
                elif isinstance(u, dict):
                    usage_raw = u
                else:
                    # 兜底：尝试把 Pydantic/命名元组类转成 dict
                    try:
                        usage_raw = dict(u)  # type: ignore
                    except Exception:
                        usage_raw = None

                if usage_raw:
                    # 直接读取常见字段
                    self.prefill_tokens = usage_raw.get("prompt_tokens")
                    self.decode_tokens = usage_raw.get("completion_tokens")
                    self.total_tokens = usage_raw.get("total_tokens")

                    # cache 命中统计在不同实现里字段名不完全一致，做多键回退
                    details = usage_raw.get("prompt_tokens_details") or {}
                    # 常见字段名：prompt_cache_hit_tokens / cached_tokens / cache_hit_tokens
                    ...
                    # 常见字段名：prompt_cache_miss_tokens / new_tokens / text_tokens(仅当 details 拆分时)
                    self.new_prompt_tokens = (
                        usage_raw.get("prompt_cache_miss_tokens")
                        or details.get("prompt_cache_miss_tokens")
                        or details.get("new_tokens")
                        or details.get("text_tokens")  # 某些实现会把 text_tokens 当作 miss 侧
                    )

                    # 如果 miss 没给出，但有 prefill & hit，用差值推
                    if self.new_prompt_tokens is None and (self.prefill_tokens is not None and self.cache_hit_tokens is not None):
                        try:
                            self.new_prompt_tokens = max(int(self.prefill_tokens) - int(self.cache_hit_tokens), 0)
                        except Exception:
                            pass
            ...
            for choice in choices:
                ...
                if isinstance(tool_calls_delta, list):
                    calls = tool_calls_map.setdefault(idx, [])
                    chunk_has_payload = True
                    for tc in tool_calls_delta:
                        if not isinstance(tc, dict):
                            continue
                        tc_idx = int(tc.get("index", len(calls))) if tc.get("index") is not None else len(calls)
                        while len(calls) <= tc_idx:
                            calls.append({})
                        call_entry = calls[tc_idx]
                        if "id" in tc:
                            call_entry["id"] = tc["id"]
                        if "type" in tc:
                            call_entry["type"] = tc["type"]
                        function_delta = tc.get("function")
                        if isinstance(function_delta, dict):
                            fn_entry = call_entry.setdefault("function", {})
                            if "name" in function_delta:
                                fn_entry["name"] = function_delta["name"]
                            if "arguments" in function_delta:
                                fn_entry["arguments"] = fn_entry.get("arguments", "") + function_delta["arguments"]
                                chunk_has_payload = True
                ...
                if isinstance(function_delta, dict):
                    ...
                    call_entry = calls[0]
                    call_entry.setdefault("type", "function")
                    fn_entry = call_entry.setdefault("function", {})
                    if "name" in function_delta:
                        fn_entry["name"] = function_delta["name"]
                    if "arguments" in function_delta:
                ...
                if chunk_has_payload:
                    last_chunk_wall_time = chunk_wall
                    if first_payload_wall is None:
                        first_payload_wall = chunk_wall
                    if not first_token_emitted:
                        first_token_emitted = True
                        self._notify_first_token(chunk_perf, chunk_wall)

        outputs: list[dict] = []
        output_tokens = 0

        for idx in range(n_choices):
            output = texts.get(idx, "")
            output_tokens += litellm.utils.token_counter(
                text=output,
                model=self.custom_tokenizer["identifier"] if self.custom_tokenizer is not None else self.config.name,
                custom_tokenizer=self.custom_tokenizer,
            ...
            if self.tools.use_function_calling:
                tool_calls = tool_calls_map.get(idx)
                if tool_calls:
