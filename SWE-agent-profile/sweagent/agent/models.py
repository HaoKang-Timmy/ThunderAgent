...
class GenericAPIModelConfig(PydanticBaseModel):
    ...
    """Override provider passed to litellm (e.g., `openai`, `vllm`)."""
...
class VLLMModelConfig(GenericAPIModelConfig):
    type: Literal["vllm"] = Field(description="Use a locally hosted vLLM HTTP server.")
    api_base: str = Field(default="http://localhost:8000/v1", description="Base URL of the vLLM OpenAI-compatible server.")
...
class LiteLLMModel(AbstractModel):
    ...
    def _single_query(
        ...
        if isinstance(self.config, VLLMModelConfig):
