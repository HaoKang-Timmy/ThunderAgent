...
- **Config-only integration:** No agent logic changes—pointing `model.model_kwargs.api_base` at ThunderAgent is enough to swap from raw vLLM to the ThunderAgent router.

To adapt: mirror the `program_id` injection and `/programs/release` call in your own batch driver, and set `api_base` to your ThunderAgent endpoint instead of directly to vLLM.
